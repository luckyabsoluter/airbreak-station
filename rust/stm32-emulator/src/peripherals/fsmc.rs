...
use crate::{
    ext_devices::{ExtDevice, ExtDevices},
    system::System,
};
...
impl Fsmc {
    ...
    fn access(offset: u32) -> Access {
        match offset {
            ...
            0x4000_0000..=0x4fff_ffff => match offset - 0x4000_0000 {
...
impl Bank {
    ...
    fn read_data(&mut self, sys: &System, offset: u32) -> u32 {
        let v = self
            .ext_device
            .as_ref()
            .map(|d| d.borrow_mut().read(sys, offset))
            .unwrap_or_default();

        trace!(
            "{} data read at offset=0x{:08x} value=0x{:08x}",
            self.name,
            offset,
            v
        );
    ...
    fn write_data(&mut self, sys: &System, offset: u32, value: u32) {
        self.ext_device
            .as_ref()
            .map(|d| d.borrow_mut().write(sys, offset, value));

        trace!(
            "{} data write at offset=0x{:08x} value=0x{:08x}",
            self.name,
            offset,
            value
        );
