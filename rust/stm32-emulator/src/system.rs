...
fn load_memory_regions(uc: &mut Unicorn<()>, config: &Config) -> Result<()> {
    for region in &config.regions {
        debug!(
            "Mapping region start=0x{:08x} len=0x{:x} name={}",
    ...
    for patch in config.patches.as_ref().unwrap_or(&vec![]) {
        uc.mem_write(patch.start.into(), &patch.data)
            .map_err(UniErr)
            .with_context(|| format!("Failed to apply patch at addr={}", patch.start))?;
    }
...
pub fn prepare<'a, 'b>(
    ...
    let ext_devices = config
        .devices
        .unwrap_or_default()
        .into_ext_devices(&mut gpio, &framebuffers)?;
    let peripherals = Peripherals::from_svd(
        svd_device,
